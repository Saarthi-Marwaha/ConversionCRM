...
-- ─────────────────────────────────────────────
-- workspaces
-- One workspace per SaaS customer (the company using ConversionCRM)
-- ─────────────────────────────────────────────
create table if not exists workspaces (
  ...
  -- The "aha moment" feature this workspace tracks for onboarding
...
-- ─────────────────────────────────────────────
-- end_users
-- The actual users of the SaaS product (not ConversionCRM admins)
-- ─────────────────────────────────────────────
create table if not exists end_users (
  id                  uuid primary key default gen_random_uuid(),
  workspace_id        uuid not null references workspaces(id) on delete cascade,
  -- ID from the customer's own system (e.g. their user ID)
...
-- ─────────────────────────────────────────────
-- events
-- Raw behavioral events ingested via the widget
-- ─────────────────────────────────────────────
...
-- ─────────────────────────────────────────────
-- engagement_scores
-- Point-in-time score snapshots (cron writes here daily)
-- ─────────────────────────────────────────────
...
-- ─────────────────────────────────────────────
-- subscriptions
-- Lemon Squeezy subscription state per end user
-- ─────────────────────────────────────────────
...
-- Workspace owners can read/write their own workspace
create policy "workspace_owner_access" on workspaces
  for all using (owner_id = auth.uid());

-- Workspace owners can access their end users
create policy "end_users_workspace_owner" on end_users
  for all using (
    workspace_id in (select id from workspaces where owner_id = auth.uid())
  );

-- Workspace owners can access events for their workspace
